    ...
    <div>
      ...
      <section className="bg-[#1a3a2a] text-white py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="inline-flex items-center gap-2 bg-[#c9a84c]/20 border border-[#c9a84c]/40 rounded-full px-4 py-1.5 mb-6">
            ...
            <span className="text-xs font-semibold tracking-widest uppercase text-[#c9a84c]">{t('partners.badge')}</span>
          </div>
          <h1 className="text-5xl md:text-6xl font-bold mb-4">
            {t('partners.titleStart')} <span className="italic text-[#c9a84c]">{t('partners.titleHighlight')}</span>
          </h1>
      ...
      <section className="py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3 mb-10">
            <div className="bg-[#1a3a2a] p-2.5 rounded-lg">
              <Building size={24} className="text-[#c9a84c]" />
            </div>
            <div>
              <h2 className="text-2xl font-bold text-[#1a3a2a]">{t('partners.medicalExcellence')}</h2>
            ...
            {medicalPartners.map((p) => (
              <Link key={p.name} to={p.path} className="bg-white rounded-2xl border border-gray-100 p-6 hover:shadow-xl transition-all group">
                <div className="w-20 h-20 bg-gray-50 rounded-xl flex items-center justify-center mb-4 border border-gray-100">
                  <span className="text-[#1a3a2a] font-bold text-sm text-center">{p.name}</span>
                </div>
                <h3 className="font-bold text-lg text-[#1a3a2a] group-hover:text-[#c9a84c] transition-colors">{p.name}</h3>
                <p className="text-[#c9a84c] text-xs font-semibold uppercase tracking-wider mb-3">{t(`partners.medical.${p.key}.subtitle`)}</p>
                ...
                <div className="flex items-start gap-2 text-gray-400 italic text-xs border-t border-gray-100 pt-4">
                  <Quote size={14} className="text-[#c9a84c] shrink-0 mt-0.5" />
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3 mb-10">
            <div className="bg-[#1a3a2a] p-2.5 rounded-lg">
              <Hotel size={24} className="text-[#c9a84c]" />
            </div>
            <div>
              <h2 className="text-2xl font-bold text-[#1a3a2a]">{t('partners.luxuryHospitality')}</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
            {hotelPartners.map((h) => (
              <Link key={h.name} to={h.path} className="bg-white rounded-xl p-5 border border-gray-100 hover:shadow-lg transition-shadow">
                <div className="w-14 h-14 bg-[#1a3a2a]/5 rounded-lg flex items-center justify-center mb-3">
                  <Hotel size={20} className="text-[#c9a84c]" />
                </div>
                <h3 className="font-bold text-[#1a3a2a] text-sm">{h.name}</h3>
                <p className="text-[#c9a84c] text-xs font-medium mb-2">{t(`partners.hotels.${h.key}.tag`)}</p>
