      ...
      <section className="bg-[#1a3a2a] text-white py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="inline-flex items-center gap-2 bg-[#c9a84c]/20 border border-[#c9a84c]/40 rounded-full px-4 py-1.5 mb-6">
            ...
            <span className="text-xs font-semibold tracking-widest uppercase text-[#c9a84c]">{t('process.badge')}</span>
          </div>
          <h1 className="text-5xl md:text-6xl font-bold mb-4">
            {t('process.titleStart')}<br />
            <span className="italic text-[#c9a84c]">{t('process.titleHighlight')}</span>
          </h1>
        </div>
      </section>
              ...
              <div key={step.key} className={`flex flex-col ${i % 2 === 0 ? 'lg:flex-row' : 'lg:flex-row-reverse'} gap-8 items-center`}>
                <div className="lg:w-1/2">
                  <img
                    ...
                    onError={(e) => { (e.target as HTMLImageElement).src = `https://placehold.co/600x400/1a3a2a/c9a84c/png?text=${encodeURIComponent(t(`process.steps.${step.key}.title`))}` }}
                  />
                </div>
                <div className="lg:w-1/2">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div className="bg-[#c9a84c]/10 px-3 py-1 rounded-full">
                      <span className="text-[#c9a84c] text-xs font-semibold uppercase tracking-wider">{t(`process.steps.${step.key}.phase`)}</span>
                    </div>
                  </div>
                  <h3 className="text-2xl font-bold text-[#1a3a2a] mb-3">{t(`process.steps.${step.key}.title`)}</h3>
