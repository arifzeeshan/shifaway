...
export default function Navbar() {
  ...
  return (
    <>
      <nav className="bg-[#047857] text-white sticky top-0 z-50 shadow-lg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden lg:flex items-center gap-6">
              {navLinks.map((link) => (
                <Link
                  key={link.path}
                  to={link.path}
                  className={`text-xs font-semibold tracking-wider uppercase transition-colors hover:text-[#f4c542] ${
                    matchesRoute(link.path) ? 'text-[#f4c542] underline underline-offset-4' : 'text-white'
                  }`}
                >
                  {link.label}
                </Link>
            ...
            <div className="hidden lg:flex items-center gap-3">
              ...
              <Link
                to="/contact"
                className="bg-[#f4c542] text-[#064e3b] px-5 py-2 rounded text-xs font-bold tracking-wider uppercase hover:bg-[#d9a520] transition-colors"
              >
                {t('nav.estimate')}
              </Link>
            </div>
            ...
            <div className="flex items-center gap-2 lg:hidden">
              <button
                className={`flex h-9 w-9 items-center justify-center rounded-full border border-[#f4c542]/40 transition-colors ${
                  moreOpen || isMoreActive ? 'bg-[#f4c542] text-[#064e3b]' : 'bg-white/10 text-white'
                }`}
                type="button"
      ...
      {moreOpen && (
        <div className="lg:hidden">
          ...
          <div
            ...
            aria-modal="true"
            aria-label={t('footer.navigation')}
            className="fixed inset-x-0 bottom-20 z-[60] rounded-t-3xl border-t border-[#f4c542]/30 bg-[#047857] px-4 pb-6 pt-5 text-white shadow-2xl"
          >
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-bold uppercase tracking-wider transition-colors ${
                      active ? 'bg-[#f4c542] text-[#064e3b]' : 'bg-white/10 text-white hover:bg-white/20'
                    }`}
                  >
                    <Icon size={18} />
                    {link.label}
      ...
      <nav className="fixed inset-x-0 bottom-0 z-50 border-t border-[#059669] bg-[#047857] px-2 pt-2 text-white shadow-[0_-8px_24px_rgba(0,0,0,0.18)] pb-[calc(0.5rem+env(safe-area-inset-bottom))] lg:hidden" aria-label={t('footer.navigation')}>
        <div className="mx-auto grid max-w-lg grid-cols-5 items-end gap-1">
            ...
            return (
              <Link
                ...
                className={`relative flex min-h-14 flex-col items-center justify-center gap-1 rounded-xl px-1 text-[10px] font-bold uppercase tracking-wide transition-colors ${
                  active ? 'text-[#f4c542]' : 'text-white/80 hover:text-white'
                }`}
              >
                {active && <span className="absolute top-0 h-0.5 w-8 rounded-full bg-[#f4c542]" />}
                <Icon size={19} />
          ...
          <Link to="/contact" className="flex min-h-14 flex-col items-center justify-end gap-1 text-[10px] font-black uppercase tracking-wide text-[#f4c542]">
            <span className="flex h-14 w-14 -translate-y-3 items-center justify-center rounded-full border-4 border-[#047857] bg-[#f4c542] text-[#064e3b] shadow-lg shadow-black/20">
              <Award size={24} />
            </span>
